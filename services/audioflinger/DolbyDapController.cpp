...
constexpr effect_uuid_t kDapImplementationUuid = {
...
bool DolbyDapController::isDapEffect(const sp<IAfEffectModule>& effect) {
    return isSupported() && effect != nullptr
            && effect->sessionId() == AUDIO_SESSION_OUTPUT_MIX
            && std::memcmp(
                    &effect->desc().type, &kDapTypeUuid, sizeof(effect_uuid_t)) == 0
            && std::memcmp(&effect->desc().uuid, &kDapImplementationUuid,
                    sizeof(effect_uuid_t)) == 0;
}

bool DolbyDapController::isSupported() {
#ifndef ENABLE_LEGACY_DAP_INTEGRATION
    return false;
#else
    // Product read-only properties describe the private effect ABI, not the
    // kernel/HAL version or simply the presence of a Dolby implementation.
...
void DolbyDapController::updateAudioTracks(audio_io_handle_t io, ActiveTrackState state) {
    ...
    if (status == NO_ERROR) {
        ...
        // The vendor may have applied the flags before the reply failed. The
        // previous ACK cannot suppress a corrective write back to that value.
        mLastAudioFlags.reset();
